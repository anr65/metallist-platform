...
const pageIDs = new Set(['overview', 'account', ...nav.map(([id]) => id)]);
function pageFromLocation() { const id = window.location.hash.slice(1); return pageIDs.has(id) ? id : 'overview'; }
...
function AccountPage() { const [error, setError] = useState(''), [busy, setBusy] = useState(false), [done, setDone] = useState(false); async function submit(event) { event.preventDefault(); const form = new FormData(event.currentTarget); setBusy(true); setError(''); try { await api('/api/password', Object.fromEntries(form)); setDone(true); } catch (e) { setError(e.message); } finally { setBusy(false); } } return <><div className="page-heading"><h1>Настройки входа</h1><p>После смены пароля все прежние сеансы будут завершены.</p></div><Card className="max-w-xl"><CardHeader><CardTitle>Сменить пароль</CardTitle><CardDescription>Новый пароль должен состоять минимум из 16 символов.</CardDescription></CardHeader><CardContent>{done ? <Alert><AlertDescription>Пароль изменён. Войдите заново, чтобы продолжить.</AlertDescription></Alert> : <form className="grid gap-5" onSubmit={submit}><label className="grid gap-2 text-sm font-medium">Текущий пароль<Input name="current" type="password" autoComplete="current-password" required /></label><label className="grid gap-2 text-sm font-medium">Новый пароль<Input name="new" type="password" autoComplete="new-password" minLength="16" required /></label>{error && <Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert>}<Button type="submit" disabled={busy}>{busy ? 'Сохранение…' : 'Сохранить новый пароль'}</Button></form>}</CardContent></Card></>; }

function App() {
  const [user, setUser] = useState(null); const [report, setReport] = useState(null); const [page, setPage] = useState(pageFromLocation); const [error, setError] = useState(''); const content = useRef(null);
  async function load() { try { const me = await api('/api/me'); setUser(me); setReport(await api('/api/report')); } catch { setUser(null); } }
  useEffect(() => { load(); }, []);
  useEffect(() => { const syncPage = () => setPage(pageFromLocation()); window.addEventListener('popstate', syncPage); window.addEventListener('hashchange', syncPage); return () => { window.removeEventListener('popstate', syncPage); window.removeEventListener('hashchange', syncPage); }; }, []);
  ...
  const go = id => { if (!pageIDs.has(id)) return; setPage(id); setError(''); const nextURL = id === 'overview' ? `${window.location.pathname}${window.location.search}` : `#${id}`; window.history.pushState(null, '', nextURL); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  ...
  const Nav = ({ mobile = false }) => <nav className="grid gap-1">{nav.map(([id, label, Icon]) => <NavButton key={id} id={id} label={label} Icon={Icon} mobile={mobile} />)}{mobile && <NavButton id="account" label="Настройки входа" Icon={Settings} mobile />}</nav>;
