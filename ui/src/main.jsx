...
function App() {
  ...
  const section = page?.startsWith('requests/') ? 'requests' : page;
  const sectionLabel = nav.find(([id]) => id === section)?.[1] || 'Настройки входа';
  const currentLabel = page === 'requests/new' ? 'Сформировать реестр' : page?.startsWith('requests/') ? 'Запрос' : sectionLabel;
  const parentPage = page?.startsWith('requests/') ? 'requests' : allowedPages(user.Role).includes('overview') ? 'overview' : null;
  const parentLabel = page?.startsWith('requests/') ? 'Карты к оплате' : 'Обзор';
