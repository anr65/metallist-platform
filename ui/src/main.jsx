...
import { Audit, Reports } from './pages';
import { CatalogPage } from './catalog';
import { MoneyPage } from './money';
import { Balances } from './balances';
import { PaymentRequests, RegistryUpload } from './forms';
...
const createActions = {
  requests: { route: 'requests/new', label: 'Сформировать реестр', compactLabel: 'Создать' },
  registries: { route: 'registries/new', label: 'Создать реестр', compactLabel: 'Создать' },
  merchants: { route: 'merchants/new', label: 'Добавить мерчанта', compactLabel: 'Добавить' },
  money: { route: 'money/new', label: 'Создать операцию', compactLabel: 'Создать' },
};
...
function App() {
  ...
  const NavButton = ({ id, label, Icon, mobile }) => { const button = <Button variant="ghost" data-active={page === id || (['requests', 'registries', 'merchants', 'money', 'catalog'].includes(id) && page?.startsWith(`${id}/`))} className="nav-button justify-start" onClick={() => go(id)}><Icon className="size-4" />{label}</Button>; return mobile ? <SheetClose asChild>{button}</SheetClose> : button; };
