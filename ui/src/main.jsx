...
function App() {
  ...
  const pages = { requests: <PaymentRequests role={user.Role} />, registries: <RegistryUpload role={user.Role} />, expenses: <><ExpenseForm /><DraftActivity role={user.Role} /></>, money: <><MoneyForm role={user.Role} /><Money /><DraftActivity role={user.Role} kind="money" title="Черновики и проведённые операции" /></>, catalog: <><Catalog /><CatalogManager role={user.Role} /><CardPANCorrection role={user.Role} /><UserManager role={user.Role} /><TelegramLinkManager role={user.Role} /></>, reports: <Reports role={user.Role} />, audit: <Audit />, account: <AccountPage /> };
