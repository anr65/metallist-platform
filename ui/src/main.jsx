...
function App() {
  ...
  const NavButton = ({ id, label, Icon, mobile }) => { const button = <Button variant="ghost" data-active={page === id || (['requests', 'registries', 'merchants'].includes(id) && page?.startsWith(`${id}/`))} className="nav-button justify-start" onClick={() => go(id)}><Icon className="size-4" />{label}</Button>; return mobile ? <SheetClose asChild>{button}</SheetClose> : button; };
