...
`GET /api/catalog` для главного администратора и операциониста включает `request_cards`: активные карты в порядке распределения строк с `full_name`/`phone` из последнего снимка реестра по карте. Поля независимы в форме; перед формированием XLSX новые комбинации сохраняются через `payment_contact`. Повторная комбинация возвращает существующий активный контакт без UPDATE и без расширения прав прикладной роли.
