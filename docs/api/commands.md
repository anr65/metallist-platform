...
`POST /api/payment-request/create` принимает `mode: "cards"`, `merchant_id`, `external_ref` и от 1 до 500 `rows` с `card_id` и `contact_id`. Сервер проверяет действующие карты и контакты, уникальность карты внутри запроса и отклоняет поля суммы. `GET /api/payment-requests` и `GET /api/payment-request/rows` возвращают состав карт без плановых сумм. XLSX нового запроса также не содержит сумму: её назначает мерчант в ответном реестре. Исторические запросы и их ранее созданные файлы не переписываются. Автоподбор по количеству выполняется в интерфейсе и передаёт в API такой же явный состав.