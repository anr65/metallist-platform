...
- `banks(id, code, name)`
- `cards(id, bank_id, last4, mask, owner_label, status)`; полный PAN не хранится в основной таблице: отдельный зашифрованный файл привязан к `card_id`, ключ находится вне БД и Git
...
Полный PAN требуется в бизнес-карточке, но не является открытым полем основной таблицы `cards`: используется зашифрованный файл по неизменному `card_id`. PIN не сохраняется нигде.
