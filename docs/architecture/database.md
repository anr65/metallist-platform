...
В демонстрационной схеме 002 есть `payment_requests(id, merchant_id, external_ref, mode, payment_count, per_payment_cents, requested_total_cents, export_path, export_sha256, created_by, created_at)` и `payment_request_rows(id, request_id, row_no, card_id, planned_cents, synthetic_number, synthetic_name)`. Схема 006 расширяет `payment_requests.mode` значением `manual` для запросов с явно выбранными уникальными картами и индивидуальными суммами строк. Схема 005 добавляет справочник `payment_contacts(id, full_name, phone, active, created_by, created_at)` и nullable-поля снимка `payment_request_rows(contact_id, contact_name, contact_phone)`. Старые строки остаются допустимыми без выдуманного телефона; приложение требует контакт для каждой новой строки. Снимок ФИО и телефона не зависит от дальнейшего состояния справочника. Пара `(merchant_id, external_ref)` и номер строки в запросе уникальны. `registries.payment_request_id` необязателен; ответный реестр проверяет совпадение мерчанта. Запрос хранит неизменяемый план и контрольную сумму XLSX, не создаёт проводок. `synthetic_number`, `contact_name` и `contact_phone` в demo содержат только вымышленные данные; production PAN и реальные персональные данные здесь хранить запрещено до отдельной архитектуры защищённого хранилища.
