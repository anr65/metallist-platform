...
В демонстрационной схеме 002 есть `payment_requests(id, merchant_id, external_ref, mode, payment_count, per_payment_cents, requested_total_cents, export_path, export_sha256, created_by, created_at)` и `payment_request_rows(id, request_id, row_no, card_id, planned_cents, synthetic_number, synthetic_name)`. Схема 005 добавила справочник `payment_contacts` и снимок контакта в строках; схема 006 добавила ручной режим. Схема 007 вводит режим `cards` для новых запросов **без суммы**: три денежных столбца nullable и обязаны быть `NULL` в новых запросах; для исторических режимов они остаются заполненными. Старые записи и XLSX не изменяются. Пара `(merchant_id, external_ref)` и номер строки в запросе уникальны. `registries.payment_request_id` необязателен; ответный реестр проверяет мерчанта, а суммы задаются только в ответном реестре. Запрос хранит неизменяемый состав карт и контрольную сумму XLSX, не создаёт проводок. В demo используются только вымышленные данные.
