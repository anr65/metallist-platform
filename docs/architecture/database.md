...
Схема 014 добавляет `payment_requests.title`: отдельное отображаемое название запроса (1–120 символов). Историческим запросам копируется `external_ref`; номер, связи, строки и проводки не меняются. Название не обязано быть уникальным.
