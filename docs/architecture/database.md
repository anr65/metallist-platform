...
Реализованная миграция 016 добавляет `registries.payment_date date`. Для новых реестров дата фактических оплат обязательна при создании; опубликованная дата не редактируется на месте. У старых реестров поле остаётся `NULL`: фактический день нельзя надёжно вывести из времени подтверждения. Старые проводки не переписываются; при перерасчёте старого реестра временно используется московская дата `confirmed_at`. Старый preview без даты нужно создать заново. Миграция добавочная; перед её применением на рабочей БД нужны резервная копия и проверка версии 15, после — проверка версии 16 и числа реестров без даты.

Миграция 017 разрешает повторную загрузку тех же байтов после сторнирования: снимает `UNIQUE` с `source_documents.sha256`, оставляет индекс для поиска и отдельную уникальность для ручных источников. Каждая повторная загрузка получает собственный `source_documents.id` и неизменяемый файл по отдельному пути; новый реестр хранит `replaces_registry_id` со ссылкой на сторнированный. Приложение сериализует загрузки одинакового SHA в транзакции и отклоняет файл, если есть активный preview или опубликованный реестр. Миграция кратко блокирует записи в `source_documents` при снятии ограничения; применять в окно обновления приложения после резервной копии. Проверка после миграции: версия 17, индекс `source_documents_sha256_idx`, отсутствие двух активных реестров для одного SHA. При откате к старому приложению схема совместима с чтением, но возврат ограничения `UNIQUE` требует сначала разобрать повторные источники; автоматически откатывать 017 нельзя.

Миграция 018 расширяет только `telegram_dialogs.command`: разрешает значение `transfer` наряду с `withdraw` и `expense`. Денежные таблицы и старые записи не изменяет. Требует версию 17 и кратковременно блокирует запись диалогов на время замены ограничения. После применения проверить версию 18, наличие трёх разрешённых команд и сохранение прежнего числа `journal_entries`/`postings`. Новая версия приложения требует эту миграцию до включения `/transfer`.
...
- Для исправления ручной ставки после подтверждения реестра нужна отдельная от изменения общего тарифа версия решения с `registry_id`, прежней/новой ставкой и суммами, preview баланса до/после, причиной, автором и временем подтверждения, идемпотентным ключом и ссылками на новые корректирующие `journal_entries`. Конкретная таблица — технический проект; исходное одобрение и postings сохраняются. Для P&L корректирующая запись относится к `payment_date` исходного реестра (полночь `Europe/Moscow`), а фактическое время публикации хранится отдельно; при затронутом утверждённом отчёте требуется новая версия и повторное утверждение бухгалтером. При последующем откате реестра необходимо учитывать и эти корректировки, чтобы итоговые комиссия и долг отменённого реестра стали нулевыми.
- `banks(id, code, name, source, external_id nullable, selectable, last_synced_at nullable)`; `source='nspk_sbp'` с уникальным `external_id` связывает запись с официальным перечнем участников СБП, исторические локальные записи сохраняются для прежних карт; `selectable=false` исключает устаревшую или условную запись из выбора при создании новой карты
- `cards(id, bank_id, last4, mask, owner_label, status, pan_ciphertext nullable)`; исходный полный PAN хранится отдельным зашифрованным файлом, исправленный — в шифртексте таблицы; ключ находится вне БД и Git. Все активные карты доступны всем активным сборщикам и операционистам без индивидуального назначения. Историческая `card_assignments` после миграции 013 сохраняется только для совместимости отката и не участвует в авторизации
...
Полный PAN требуется в бизнес-карточке, но не является открытым полем основной таблицы `cards`: исходный номер хранится в зашифрованном файле по неизменному `card_id`, исправленный — в зашифрованном `pan_ciphertext`. PIN не сохраняется нигде.

## Документы и импорт

В демонстрационной схеме 002 есть `payment_requests(id, merchant_id, external_ref, mode, payment_count, per_payment_cents, requested_total_cents, export_path, export_sha256, created_by, created_at)` и `payment_request_rows(id, request_id, row_no, card_id, planned_cents, synthetic_number, synthetic_name)`. Схема 005 добавила справочник `payment_contacts` и снимок контакта в строках; схема 006 добавила ручной режим. Схема 007 вводит режим `cards` для новых запросов **без суммы**: три денежных столбца nullable и обязаны быть `NULL` в новых запросах; для исторических режимов они остаются заполненными. Старые записи и XLSX не изменяются. Пара `(merchant_id, external_ref)` и номер строки в запросе уникальны. `registries.payment_request_id` необязателен; ответный реестр проверяет мерчанта, а суммы задаются только в ответном реестре. Схема 008 разрешает `NULL` в старых полях пути/хеша XLSX и учебного номера/имени: новый запрос хранит состав карт и снимок контакта, а полный PAN берётся из закрытого зашифрованного хранилища только при скачивании. XLSX не сохраняется на сервере, экспорт аудируется по SHA-256 и не создаёт проводок.
...
Схема 014 добавляет `payment_requests.title`: отдельное отображаемое название запроса (1–120 символов). Историческим запросам копируется `external_ref`; старые команды вставки без `title` получают его из `external_ref` через триггер. Номер, связи, строки и проводки не меняются. Название не обязано быть уникальным.
