	...
	for name, q := range specs {
		...
		for rows.Next() {
			...
			if name == "cards" || name == "request_cards" {
				...
				panInDB, _ := m["pan_in_db"].(bool)
				delete(m, "pan_in_db")
				path, pathErr := vaultPath(cardID)
				_, statErr := os.Stat(path)
				panSaved := panInDB || pathErr == nil && statErr == nil
...
func (a *App) catalogCreate(w http.ResponseWriter, r *http.Request, u User) {
	...
	case "card":
		...
		var bankSelectable bool
		if bankErr := a.db.QueryRow("SELECT selectable FROM banks WHERE id=$1", bankID).Scan(&bankSelectable); bankErr != nil || !bankSelectable {
			e = errors.New("выберите действующий банк из справочника НСПК")
			break
		}
